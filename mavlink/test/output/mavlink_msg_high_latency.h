...
#define MAVLINK_MSG_ID_HIGH_LATENCY_LEN 64
#define MAVLINK_MSG_ID_HIGH_LATENCY_MIN_LEN 64
#define MAVLINK_MSG_ID_234_LEN 64
#define MAVLINK_MSG_ID_234_MIN_LEN 64

#define MAVLINK_MSG_ID_HIGH_LATENCY_CRC 102
#define MAVLINK_MSG_ID_234_CRC 102

#define MAVLINK_MSG_HIGH_LATENCY_FIELD_PADDING_LEN 8
#define MAVLINK_MSG_HIGH_LATENCY_FIELD_TAG_LEN 16
...
#define MAVLINK_MESSAGE_INFO_HIGH_LATENCY { \
    ...
    {  { "base_mode", NULL, MAVLINK_TYPE_UINT8_T, 0, 26, offsetof(mavlink_high_latency_t, base_mode) }, \
         ...
         { "padding", NULL, MAVLINK_TYPE_UINT8_T, 8, 40, offsetof(mavlink_high_latency_t, padding) }, \
         { "tag", NULL, MAVLINK_TYPE_UINT8_T, 16, 48, offsetof(mavlink_high_latency_t, tag) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_HIGH_LATENCY { \
                               ...
                               uint8_t base_mode, uint32_t custom_mode, uint8_t landed_state, int16_t roll, int16_t pitch, uint16_t heading, int8_t throttle, int16_t heading_sp, int32_t latitude, int32_t longitude, int16_t altitude_amsl, int16_t altitude_sp, uint8_t airspeed, uint8_t airspeed_sp, uint8_t groundspeed, int8_t climb_rate, uint8_t gps_nsat, uint8_t gps_fix_type, uint8_t battery_remaining, int8_t temperature, int8_t temperature_air, uint8_t failsafe, uint8_t wp_num, uint16_t wp_distance)
{
    ...
    _mav_put_uint8_t_array(buf, 40, NULL, 8);
...
/**
 * @brief Get field padding from high_latency message
 *
 * @return Padding for encryption
 */
static inline uint16_t mavlink_msg_high_latency_get_padding(const mavlink_message_t* msg, uint8_t *padding)
{
    return _MAV_RETURN_uint8_t_array(msg, padding, 8,  40);
}

/**
 * @brief Get field tag from high_latency message
 *
 * @return 128 bit message signature
 */
static inline uint16_t mavlink_msg_high_latency_get_tag(const mavlink_message_t* msg, uint8_t *tag)
{
...
static inline void mavlink_msg_high_latency_decode(const mavlink_message_t* msg, mavlink_high_latency_t* high_latency)
{
    ...
    mavlink_msg_high_latency_get_padding(msg, high_latency->padding);
    mavlink_msg_high_latency_get_tag(msg, high_latency->tag);
#else
