...
MAVLINK_HELPER void chaskey_sign(const uint8_t* msg, const uint8_t len, uint8_t* tag)
{
...
MAVLINK_HELPER void aes_encrypt(uint8_t* cipher, const uint32_t len)
{
	...
	struct AES_ctx ctx;
	//printf("+++++++");
	//for(unsigned int i = 0; i < msglen; i++) printf("%02x", msg[i]);
	//printf("\n");
	//memcpy(cipher, msg, msglen);
	//printf("-------");
	//for(unsigned int i = 0; i < msglen; i++) printf("%02x", cipher[i]);
    //printf("\n");
...
/**
MAVLINK_HELPER void aes_decrypt(const char* msg, const uint32_t len, uint8_t* cipher)
{
	uint8_t key[] = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };
	uint8_t iv[]  = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
	uint32_t msglen = len;
