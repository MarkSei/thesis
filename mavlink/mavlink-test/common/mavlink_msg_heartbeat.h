...
#define MAVLINK_MSG_ID_HEARTBEAT_LEN 25
#define MAVLINK_MSG_ID_HEARTBEAT_MIN_LEN 25
                               ...
                               uint8_t type, uint8_t autopilot, uint8_t base_mode, uint32_t custom_mode, uint8_t system_status)
{
    ...
    uint8_t n_tag[16];
    uint8_t message[MAVLINK_MSG_ID_HEARTBEAT_LEN-16];

    uint8_t custom_mode_8[4];
    mavlink_u32_to_u8(custom_mode, custom_mode_8);

    message[0] = custom_mode_8[0];
    message[1] = custom_mode_8[1];
    message[2] = custom_mode_8[2];
    message[3] = custom_mode_8[3];

    message[4] = type;
    message[5] = autopilot;
    message[6] = base_mode;
    message[7] = system_status;
    message[8] = 3;

    chaskey_sign(message, MAVLINK_MSG_ID_HEARTBEAT_LEN-16, n_tag);

    _mav_put_uint8_t_array(buf, 9, n_tag, 16);

    //uint8_t* cipher[MAVLINK_MSG_ID_HEARTBEAT_LEN];
    //aes_encrypt(buf, MAVLINK_MSG_ID_HEARTBEAT_LEN, cipher);
    ...
    mav_array_memcpy(packet.tag, n_tag, sizeof(uint8_t)*16);
    ...
    uint8_t cipher[32];
    memset(cipher, 0x00, 32);
    //memcpy(cipher, _MAV_PAYLOAD(msg), MAVLINK_MSG_ID_HEARTBEAT_LEN-1);

    for(unsigned int i = 0; i < MAVLINK_MSG_ID_HEARTBEAT_LEN; i++) cipher[i] = (uint8_t)_MAV_PAYLOAD(msg)[i];

    for(unsigned int i = 0; i < MAVLINK_MSG_ID_HEARTBEAT_LEN; i++) printf("%02x", cipher[i]);
    printf("\n");

    //for(unsigned int i = 0; i < MAVLINK_MSG_ID_HEARTBEAT_LEN; i++) printf("%02x", cipher[i]);
    //printf("\n");
    aes_encrypt(cipher, MAVLINK_MSG_ID_HEARTBEAT_LEN-1);

    //for(unsigned int i = 0; i < 16; i++) printf("%02x", packet.tag[i]);
    //printf("\n");
    //for(unsigned int i = 0; i < MAVLINK_MSG_ID_HEARTBEAT_LEN; i++) printf("%02x", _MAV_PAYLOAD(msg)[i]); //((MAVLINK_MAX_PAYLOAD_LEN+MAVLINK_NUM_CHECKSUM_BYTES+7)/8)
    //printf("\n");
    for(unsigned int i = 0; i < MAVLINK_MSG_ID_HEARTBEAT_LEN; i++) printf("%02x", cipher[i]);
    printf("\n");
    
    //memcpy(_MAV_PAYLOAD_NON_CONST(msg), cipher, MAVLINK_MSG_ID_HEARTBEAT_LEN);
    

    //for(unsigned int i = 0; i < MAVLINK_MSG_ID_HEARTBEAT_LEN; i++) printf("%02x", _MAV_PAYLOAD(msg)[i]);
    //printf("\n");
...
static inline void mavlink_msg_heartbeat_send(mavlink_channel_t chan, uint8_t type, uint8_t autopilot, uint8_t base_mode, uint32_t custom_mode, uint8_t system_status, const uint8_t *tag)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_HEARTBEAT_LEN];
    _mav_put_uint32_t(buf, 0, custom_mode);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_heartbeat_send(chan, heartbeat->type, heartbeat->autopilot, heartbeat->base_mode, heartbeat->custom_mode, heartbeat->system_status, heartbeat->tag);
#else
 ...
 */
static inline void mavlink_msg_heartbeat_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t type, uint8_t autopilot, uint8_t base_mode, uint32_t custom_mode, uint8_t system_status, const uint8_t *tag)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
