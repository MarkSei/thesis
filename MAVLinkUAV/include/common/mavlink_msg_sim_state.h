...
#define MAVLINK_MSG_ID_SIM_STATE_LEN 112
#define MAVLINK_MSG_ID_SIM_STATE_MIN_LEN 112
#define MAVLINK_MSG_ID_108_LEN 112
#define MAVLINK_MSG_ID_108_MIN_LEN 112

#define MAVLINK_MSG_ID_SIM_STATE_CRC 212
#define MAVLINK_MSG_ID_108_CRC 212

#define MAVLINK_MSG_SIM_STATE_FIELD_PADDING_LEN 12
#define MAVLINK_MSG_SIM_STATE_FIELD_TAG_LEN 16
...
#define MAVLINK_MESSAGE_INFO_SIM_STATE { \
    ...
    {  { "q1", NULL, MAVLINK_TYPE_FLOAT, 0, 0, offsetof(mavlink_sim_state_t, q1) }, \
         ...
         { "padding", NULL, MAVLINK_TYPE_UINT8_T, 12, 84, offsetof(mavlink_sim_state_t, padding) }, \
         { "tag", NULL, MAVLINK_TYPE_UINT8_T, 16, 96, offsetof(mavlink_sim_state_t, tag) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_SIM_STATE { \
                               ...
                               float q1, float q2, float q3, float q4, float roll, float pitch, float yaw, float xacc, float yacc, float zacc, float xgyro, float ygyro, float zgyro, float lat, float lon, float alt, float std_dev_horz, float std_dev_vert, float vn, float ve, float vd)
{
    ...
    _mav_put_uint8_t_array(buf, 84, NULL, 12);
...
/**
 * @brief Get field padding from sim_state message
 *
 * @return Padding for encryption
 */
static inline uint16_t mavlink_msg_sim_state_get_padding(const mavlink_message_t* msg, uint8_t *padding)
{
    return _MAV_RETURN_uint8_t_array(msg, padding, 12,  84);
}

/**
 * @brief Get field tag from sim_state message
 *
 * @return 128 bit message signature
 */
static inline uint16_t mavlink_msg_sim_state_get_tag(const mavlink_message_t* msg, uint8_t *tag)
{
...
static inline void mavlink_msg_sim_state_decode(const mavlink_message_t* msg, mavlink_sim_state_t* sim_state)
{
    ...
    mavlink_msg_sim_state_get_padding(msg, sim_state->padding);
    mavlink_msg_sim_state_get_tag(msg, sim_state->tag);
#else
